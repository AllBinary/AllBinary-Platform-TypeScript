...
//not GWT import - same folder const PaymentGatewayPrimaryKey = globalThis.org.allbinary.business.user.commerce.money.payment.gateway.PaymentGatewayPrimaryKey;
...
paymentGatewayHashMap!.put(EntryData.getInstance()!.LASTMODIFIED.toString(), this.lastModified);
    ...
    public toVector(isEncrypted: boolean): BasicArrayList{

    var updateVector: BasicArrayList = new BasicArrayListD();;
...
updateVector!.add(this.special9);
    ...
    public getKey(): any{
                        ...
                        //if statement needs to be on the same line and ternary does not work the same way.
                        return new PaymentGatewayPrimaryKey(this.getStoreName(), this.getName());
